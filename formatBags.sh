...
for keyword in ${!keywordCorrection[@]}; do # For keyword in the associated array:
  ...
  # if it already contains correction, pass, else: add correction infront of the first instance of keyword found
    if [[ "$keyword" == "beet_" ]]; then # Specifically if the keyword we're currently on is beet:
        find ${A_TARGET_BAG_DIR} -type f \( -iname *"beet_"* -and -not -iname *"sugarbeet"*  \) | while read line; # We want to also check there's no instances of sugarbeet
        do
...
for keyword in ${removalArray[@]}; do 
    find ${A_TARGET_BAG_DIR} -type f \( -iname *"${keyword}"* \) | while read line;
    do 
        newFileName=${line//${keyword}/} # Replace //"all" instances of keyword with /(blank) Nothing.
